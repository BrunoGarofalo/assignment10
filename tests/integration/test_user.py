...
def test_user_queries(db_session, seed_users):
    """Apply count, filter, order queries"""
...
def test_generate_access_token_default_expiration():
    ...
    assert ACCESS_TOKEN_EXPIRE_MINUTES - 1 <= delta_minutes <= ACCESS_TOKEN_EXPIRE_MINUTES + 1
...
def test_generate_access_token_custom_expiration():
    ...
    now = datetime.utcnow()
    delta_minutes = (exp - now).total_seconds() / 60
    assert 9 <= delta_minutes <= 11 

def test_verify_token_valid():
    # Generate a valid token for a fake UUID
    user_id = uuid.uuid4()
    token = User.generate_access_token(str(user_id))
...
def test_verify_token_invalid():
    ...
    decoded_id = User.verify_token(invalid_token)
